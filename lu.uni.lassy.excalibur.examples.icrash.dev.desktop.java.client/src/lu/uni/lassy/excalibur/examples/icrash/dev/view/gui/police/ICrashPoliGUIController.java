...
import lu.uni.lassy.excalibur.examples.icrash.dev.controller.PoliceController;
 ...
 */
/**
 * The Class ICrashGUIController, used for dealing with the GUI for the Police.
 */
public class ICrashPoliGUIController extends AbstractAuthGUIController {
	/*
	...
	*/
    ...
    @FXML
    private TableView<Message> tblvwPoliMessages;

    /** The button that allows a user to logoff. */
    @FXML
    private Button bttnPoliLogoff;
     ...
     */
    @FXML
    void bttnPoliLogoff_OnClick(ActionEvent event) {
    	logoff();
    }
     ...
     */
    @FXML
    void bttnPoliLogon_OnClick(ActionEvent event) {
    	logon();
    }
	...
	/** The user controller, for this GUI it's the coordinator controller and allows access to Police functions like reporting on crises. */
	private PoliceController userController;
	...
	public void setUpTables(){
		setUpMessageTables(tblvwPoliMessages);
		setUpCrisesTablesPoli(tblvwCrisis);
	...
	@Override
	public void logon() {
		if(txtfldPoliLogonUserName.getText().length() > 0 && psswrdfldPoliLogonPassword.getText().length() > 0){
			try {
				if (userController.oeLogin(txtfldPoliLogonUserName.getText(), psswrdfldPoliLogonPassword.getText()).getValue()){
					if (userController.getUserType() == UserType.Police){
						logonShowPanes(true);
					}
				}
			}
	...
	protected void logonShowPanes(boolean loggedOn){
		...
		bttnPoliLogoff.setDisable(!loggedOn);
		pnLogon.setVisible(!loggedOn);
		bttnPoliLogon.setDefaultButton(!loggedOn);
		if (loggedOn){
			tbpnMain.getSelectionModel().selectFirst();
			cmbbxCrisisStatus.setValue(EtCrisisStatus.pending);
		}
		else{
			txtfldPoliLogonUserName.setText("");
			psswrdfldPoliLogonPassword.setText("");
			txtfldPoliLogonUserName.requestFocus();
	...
	public PtBoolean setActor(JIntIsActor actor) {
		try { 
			if (actor instanceof ActPolice)
			{
				try{
					userController = new PoliceController((ActPolice)actor);
					try{
						userController.getAuthImpl().listOfMessages.addListener(new ListChangeListener<Message>() {
							...
							public void onChanged(ListChangeListener.Change<? extends Message> c) {
								addMessageToTableView(tblvwPoliMessages, c.getList());
							}
						});
						
						((ActProxyPoliceImpl)userController.getAuthImpl()).MapOfCtCrisis.addListener(new MapChangeListener<String, CtCrisis>(){
				...
				throw new IncorrectActorException(actor, ActPolice.class);
