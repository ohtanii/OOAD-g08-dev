...
public class SystemStateController {
	 ...
	 *
	 * @param userName the user name
	 * @return the act police
	 * @throws ServerNotBoundException is only thrown when attempting to access a server which has no current binding. This shouldn't happen, but you never know!
	 * @throws ServerOfflineException is an error that is thrown when the server is offline or not reachable
	 */
	public ActPolice getActPolice(String userName) throws ServerNotBoundException, ServerOfflineException{
		try {
			return server.env().getActPolice(userName);
		} catch (RemoteException e) {
			Log4JUtils.getInstance().getLogger().error(e);
			throw new ServerOfflineException();
		} catch (NotBoundException e) {
	...
	/**
	 * Gets an ArrayList of all CtPolices in the system.
	 *
	 * @return An ArrayList of all CtPolices from the server
	 * @throws ServerOfflineException is an error that is thrown when the server is offline or not reachable
	 * @throws ServerNotBoundException is only thrown when attempting to access a server which has no current binding. This shouldn't happen, but you never know!
	 */
	public ArrayList<CtPolice> getAllPolices() throws ServerOfflineException, ServerNotBoundException{
		try {
			return server.sys().getAllCtPolices();
		} catch (RemoteException e) {
			Log4JUtils.getInstance().getLogger().error(e);
			throw new ServerOfflineException();
		} catch (NotBoundException e) {
