...
public class ICrashCoordGUIController extends AbstractAuthGUIController {
	...
	*/
    ...
    /** The button that allows a user to change crisis type to huge. */
    @FXML
    private ComboBox <EtCrisisType> cmbbxCrisisType;
	...
	private void changeCrisisType(){
		...
		if (crisis != null){
			...
			if (result.isPresent()){
				if (!result.get().getValue())
					showWarningMessage("Unable to change status of type", "Unable to change type of crisis, please try again");
			}
		}
		populateCrisis();
	}
