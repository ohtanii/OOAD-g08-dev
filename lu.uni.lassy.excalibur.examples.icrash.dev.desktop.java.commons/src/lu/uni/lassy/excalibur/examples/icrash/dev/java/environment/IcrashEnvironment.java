...
public interface IcrashEnvironment  extends Remote {
	...
	// POlICE
	/**
	 * Puts an actor Police into the hashtable of the system.
	 *
	 * @param keyName The key of which to store the actor by, keeps this the same as that you use the get actor method. Currently we use the login name as the key
	 * @param aActPolice The actor Police stored in the hashtable
	 * @throws RemoteException Thrown if the server is offline
	 */
	public void setActPolice(String keyName, ActPolice aActPolice) throws RemoteException;
	
	/**
	 * Gets the actor Police from the hashtable using the key value provided.
	 *
	 * @param keyName The login name of the Police to retrieve
	 * @return The actor retrieved from the database, can be null if one does not exist within the hashtable with the correct key
	 * @throws RemoteException Thrown if the server is offline
	 */
	public ActPolice getActPolice(String keyName) throws RemoteException;
