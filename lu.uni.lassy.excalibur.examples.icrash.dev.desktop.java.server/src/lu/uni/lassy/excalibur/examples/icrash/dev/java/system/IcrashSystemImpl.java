...
import lu.uni.lassy.excalibur.examples.icrash.dev.java.environment.actors.ActPoliceImpl;
...
import lu.uni.lassy.excalibur.examples.icrash.dev.java.system.db.DbPolices;
...
public class IcrashSystemImpl extends UnicastRemoteObject implements
	...
	/**  A hashtable of the joint class polices and actors polices in the system, stored by their class type. */
	Hashtable<CtPolice, ActPolice> assCtPoliceActPolice = new Hashtable<CtPolice, ActPolice>();
	
	/**  A hashtable of the joint crises and coordinators in the system, stored by their crisis as a key. */
	Hashtable<CtCrisis, CtPolice> assCtCrisisCtPolice = new Hashtable<CtCrisis, CtPolice>();
	...
	/**
	 * Gets the class Authenticated (Of a police type) that has the associated ID provided.
	 *
	 * @param aDtPoliceID The ID to use to search for the user
	 * @return the class Authenticated that was associated with the ID provided, if none found then return a null
	 */
	public CtAuthenticated getCtPolice(DtPoliceID aDtPoliceID) {

		for (CtAuthenticated ctAuth : assCtAuthenticatedActAuthenticated
				.keySet()) {
			if (ctAuth instanceof CtPolice) {
				PtBoolean res = ((CtPolice) ctAuth).id
						.eq(aDtPoliceID);
				if (res.getValue())
					return ctAuth;
			}
	 ...
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#getRandomCtCoordinator()
	 */
	public CtPolice getRandomCtPolice() throws RemoteException {
		List<CtPolice> collCtPoli = assCtPoliceActPolice.keySet().stream().filter(t -> t != null).collect(Collectors.toList());
		int max = collCtPoli.size();
		int min = 0;
		int randomNum = 0;
		...
		if (max != 0){
			return collCtPoli.get(randomNum);
		}
		return null;
	}
	 ...
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#getActCoordinator(lu.uni.lassy.excalibur.examples.icrash.dev.java.system.types.primary.CtCoordinator)
	 */
	public ActPolice getActPolice(CtPolice keyCtPolice)
			throws RemoteException {
		return assCtPoliceActPolice.get(keyCtPolice);
	}

	/* (non-Javadoc)
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#bindCtCrisisCtCoordinator(lu.uni.lassy.excalibur.examples.icrash.dev.java.system.types.primary.CtCrisis, lu.uni.lassy.excalibur.examples.icrash.dev.java.system.types.primary.CtCoordinator)
	 */
	public void bindCtCrisisCtPolice(CtCrisis aCtCrisis,
			CtPolice aCtPolice) throws RemoteException {
		assCtCrisisCtPolice.put(aCtCrisis, aCtPolice);
		DbCrises.bindCrisisPolice(aCtCrisis, aCtPolice);
		DbCrises.updateCrisis(aCtCrisis);
	}
///////
	 ...
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#getAllCtCoordinators()
	 */
	public ArrayList<CtPolice> getAllCtPolices() throws RemoteException {
		ArrayList<CtPolice> result = new ArrayList<CtPolice>();
		for (CtAuthenticated auth : cmpSystemCtAuthenticated.values())
			if (auth instanceof CtPolice)
				result.add((CtPolice) auth);
		return result;
	}
	...
	public PtBoolean oeCreateSystemAndEnvironment(PtInteger aQtyComCompanies)
		...
		try {
			...
			*/
			...
			Hashtable<String, CtPolice> cmpSystemCtPolice = DbPolices.getSystemPolices();
			for(CtPolice ctPoli: cmpSystemCtPolice.values()){
				cmpSystemCtAuthenticated.put(ctPoli.login.value.getValue(), ctPoli);
				ActPolice actPoli = new ActPoliceImpl(ctPoli.login);
				env.setActPolice(ctPoli.login.value.getValue(), actPoli);
				assCtAuthenticatedActAuthenticated.put(ctPoli, actPoli);
				assCtPoliceActPolice.put(ctPoli, actPoli);
			}
			...
			assCtCrisisCtPolice = DbCrises.getAssCtCrisisCtPolice();
	...
	public  synchronized PtBoolean oeSetCrisisStatus(DtCrisisID aDtCrisisID,
		...
		try{
			...
			if (currentRequestingAuthenticatedActor instanceof ActPolice) {
				ActPolice theActPolice = (ActPolice) currentRequestingAuthenticatedActor;
				//PostF1
				theCrisis.status = aEtCrisisStatus;
				DbCrises.updateCrisis(theCrisis);
				PtString aMessage = new PtString("The crisis status has been updated !");
				theActPolice.ieMessage(aMessage);
	...
	public synchronized  PtBoolean oeSetCrisisHandler(DtCrisisID aDtCrisisID)
		...
		try{
			...
			if (currentRequestingAuthenticatedActor instanceof ActPolice) {
				ActPolice theActPolice = (ActPolice) currentRequestingAuthenticatedActor;
				CtPolice theCtPolice = (CtPolice) getCtAuthenticated(theActPolice);
				
				log.debug("theCrisis Instance is " + theCrisis.toString());
				log.debug("aDtCrisisID.value.getValue() is "
						+ aDtCrisisID.value.getValue());
	
				CtPolice theCtPoliceAtPre = assCtCrisisCtPolice
						.get(theCrisis);
				if (theCtPoliceAtPre == null)
					log.debug("theCtPoliceAtPre is none");
				else
						log.debug("theCtPoliceAtPre is "
								+ assCtCrisisCtPolice.get(theCrisis).toString());
	
				//PostF1
				theCrisis.status = EtCrisisStatus.handled;
				DbCrises.updateCrisis(theCrisis);
				
				assCtCrisisCtPolice.put(theCrisis, theCtPolice);
				DbCrises.bindCrisisPolice(theCrisis, theCtPolice);
				PtString aMessage = new PtString(
						"You are now considered as handling the crisis !");
				theActPolice.ieMessage(aMessage);
				...
				//PostF3
				if (theCtPoliceAtPre != null) {
					ActPolice theActPoliceAtPre = (ActPolice) assCtAuthenticatedActAuthenticated
							.get(theCtPoliceAtPre);
					log.info("One of the crisis you were handling is now handled by one of your colleagues!");
					PtString aMessage2 = new PtString(
							"One of the crisis you were handling is now handled by one of your colleagues!");
					theActPoliceAtPre.ieMessage(aMessage2);
				}
	...
	public PtBoolean oeGetCrisisSet(EtCrisisStatus aEtCrisisStatus) {
		try{
			...
			if (currentRequestingAuthenticatedActor instanceof ActPolice) {
				ActPolice aActPolice = (ActPolice) currentRequestingAuthenticatedActor;
				//go through all existing crises
				for (String crisisKey : cmpSystemCtCrisis.keySet()) {
					CtCrisis crisis = cmpSystemCtCrisis.get(crisisKey);
					if (crisis.status.toString().equals(aEtCrisisStatus.toString()))
						//PostF1
						crisis.isSentToPolice(aActPolice);
				}
				return new PtBoolean(true);
			}
		}
	...
	public  synchronized PtBoolean oeCloseCrisis(DtCrisisID aDtCrisisID) {
		try{
			...
			if (currentRequestingAuthenticatedActor instanceof ActPolice) {
				...
				DbCrises.updateCrisis(theCrisis);
				//PostF2
				assCtCrisisCtPolice.remove(theCrisis);
	 ...
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#oeAddCoordinator(lu.uni.lassy.excalibur.examples.icrash.dev.java.system.types.primary.DtCoordinatorID, lu.uni.lassy.excalibur.examples.icrash.dev.java.system.types.primary.DtLogin, lu.uni.lassy.excalibur.examples.icrash.dev.java.system.types.primary.DtPassword)
	 */
	public PtBoolean oeAddPolice(DtPoliceID aDtPoliceID,
			DtLogin aDtLogin, DtPassword aDtPassword) throws RemoteException {
		try {
			//PreP1
			isSystemStarted();
			...
			IcrashEnvironment env = (IcrashEnvironment) registry
					.lookup("iCrashEnvironment");
			//PostF1
			ActPolice actPolice = new ActPoliceImpl(aDtLogin);
			env.setActPolice(aDtLogin.value.getValue(), actPolice);

			//PostF2
			CtPolice ctPolice = new CtPolice();
			ctPolice.init(aDtPoliceID, aDtLogin, aDtPassword);
			DbPolices.insertPolice(ctPolice);
			
			//////////////////////////////////////////////////////////////////////////////////////
			
			
			//PostF3 and PostF4 done at once w.r.t. our implementation
			assCtAuthenticatedActAuthenticated.put(ctPolice,
					actPolice);

			//Update composition relationships
			cmpSystemCtAuthenticated.put(aDtLogin.value.getValue(),
					ctPolice);
			assCtPoliceActPolice.put(ctPolice, actPolice);
			//PostF5
			ActAdministrator admin = (ActAdministrator) currentRequestingAuthenticatedActor;
			admin.iePoliceAdded();
		} catch (Exception ex) {
	 ...
	 * @see lu.uni.lassy.excalibur.examples.icrash.dev.java.system.IcrashSystem#oeDeleteCoordinator(lu.uni.lassy.excalibur.examples.icrash.dev.java.system.types.primary.DtCoordinatorID)
	 */
	public PtBoolean oeDeletePolice(DtPoliceID aDtPoliceID) throws RemoteException {
		try {
			//PreP1
			isSystemStarted();
			//PreP2
			isAdminLoggedIn();
			CtAuthenticated ctAuth = getCtPolice(aDtPoliceID);
			if (ctAuth != null && ctAuth instanceof CtPolice) {
				CtPolice aCtPolice = (CtPolice)ctAuth;
				DbPolices.deletePolice(aCtPolice);
				//PostF1
				assCtAuthenticatedActAuthenticated.remove(ctAuth);
				cmpSystemCtAuthenticated.remove(ctAuth.login.value.getValue());
				ActAdministrator admin = (ActAdministrator) currentRequestingAuthenticatedActor;
				//PostF2
				admin.iePoliceDeleted();
				return new PtBoolean(true);
			}
			return new PtBoolean(false);
		} catch (Exception e) {
	 ...
	 * 
	 * */
	public PtBoolean oeUpdatePolice(DtPoliceID aDtPoliceID,DtLogin aDtLogin,DtPassword aDtPassword) throws java.rmi.RemoteException{
		try {
			//PreP1
			isSystemStarted();
			//PreP2
			...
			if (ctAuth != null && ctAuth instanceof CtPolice){
				CtPolice aCtPolice = (CtPolice)ctAuth;
				CtPolice oldPolice = new CtPolice();
				oldPolice.init(aCtPolice.id, aCtPolice.login, aCtPolice.pwd);
				aCtPolice.update(aDtLogin, aDtPassword);
				if (DbPolices.updatePolice(aCtPolice).getValue()){
					cmpSystemCtAuthenticated.remove(oldPolice.login.value.getValue());
					cmpSystemCtAuthenticated.put(aCtPolice.login.value.getValue(), aCtPolice);
					ActAdministrator admin = (ActAdministrator) currentRequestingAuthenticatedActor;
					admin.iePoliceUpdated();
					return new PtBoolean(true);
				}
				else
					aCtPolice.update(oldPolice.login, oldPolice.pwd);
			}
			return new PtBoolean(false);
		} catch (Exception e) {
			log.error("Exception in oeDeleteCoordinator..." + e);
